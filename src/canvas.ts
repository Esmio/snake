...
const COLS =  Math.floor(FULL_WIDTH / (CELL_SIZE + GAP_SIZE));
const ROWS = COLS;
const MARGIN_TOP = Math.round((FULL_WIDTH % (CELL_SIZE + GAP_SIZE)) / 2);

const CANVAS_WIDTH = COLS * (CELL_SIZE + GAP_SIZE) - GAP_SIZE;
const CANVAS_HEIGHT = ROWS * (CELL_SIZE + GAP_SIZE) - GAP_SIZE;
...
export function renderScore(ctx: CanvasRenderingContext2D, score: number) {
  ...
  drawText(ctx, score.toString(), textX, textY, 'rgba(0,0,0,.1)', 150);
}
...
export function renderGameOver(ctx: CanvasRenderingContext2D) {
  ...
  drawText(ctx, 'GAME OVER!', textX, textY, 'black', 50);
}
...
function drawText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, fillStyle: string,
  fontSize: number, horizontalAlign: CanvasTextAlign = 'center',
